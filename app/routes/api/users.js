              ...
              newTempUser.save((err, tempuser) => {
                if (err || !tempuser) {
                  ...
                  let mail_options = {
                    ...
                    html:
                      "Hello,<br> Please Click on following link to activate your account.<br><a href=" +
                      link +
                      ">Confirm account</a>"
                  };
                  ...
                  sendEmail(mail_options, function(error, info) {
                    if (error) {
                      utils.sendJsonError(
                        res,
                        "Sending confirmation mail failed",
                        500
                      );
                    } else {
    ...
    User.findOne({ email: req.body.email }, function(err, user) {
      if (!err && user) {
        ...
        let mail_options = {
          ...
          html:
            "Hello,<br> Open Kiwi and use this token to reset password<br>token: " +
            token
        };
    ...
    jwt.verify(req.body.token, config.private_key, function(err, decoded) {
      if (!err) {
        ...
        User.findOneAndUpdate(
          ...
          (err, user) => {
            if (err || !user) {
              utils.sendJsonError(res, "Updating password failed", 404);
            } else {
